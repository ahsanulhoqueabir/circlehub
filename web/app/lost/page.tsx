...
import { useLostItems } from "@/contexts/data-context";
...
export default function LostPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (selectedDateRange > 0) {
      ...
      filtered = filtered.filter(
        (item) => new Date(item.date_lost) >= cutoffDate,
      );
    }
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-6 sm:mb-8 lg:mb-12">
          <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
            <Search className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-red-600 dark:text-red-400" />
          </div>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-5 py-3 sm:px-6 sm:py-3.5 lg:px-8 lg:py-4 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white rounded-lg font-medium transition-all hover:shadow-lg active:scale-95 min-h-[44px] text-sm sm:text-base"
          >
            ...
            {filteredItems.length === 0 ? (
              <div className="text-center py-12 sm:py-16 lg:py-20 px-4">
                <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-muted rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                  <Search className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-slate-400" />
                </div>
                ...
                {!searchQuery && (
                  <button
                    onClick={handleReportClick}
                    className="inline-flex items-center justify-center gap-2 px-5 py-3 sm:px-6 sm:py-3.5 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white rounded-lg font-medium transition-all hover:shadow-lg active:scale-95 min-h-[44px] text-sm sm:text-base"
                  >
                    <Plus className="w-4 h-4 sm:w-5 sm:h-5" />
