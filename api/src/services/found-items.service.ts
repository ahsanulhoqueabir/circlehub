...
const buildFilter = (query: FoundItemQueryInput): Record<string, unknown> => {
  ...
  if (query.dateFrom || query.dateTo) {
    ...
    filter.dateFound = dateRange;
  }

  return filter;
};

export const listFoundItems = async (query: FoundItemQueryInput) => {
  ...
  const [items, total] = await Promise.all([
    FoundItem.find(filter).sort(sort).skip(offset).limit(limit),
...
const getOwnedFoundItem = async (id: string, userId: string) => {
  if (!mongoose.Types.ObjectId.isValid(id)) {
    throw new ApiError(StatusCodes.BAD_REQUEST, "Invalid item id");
  }
...
export const updateFoundItem = async (
  id: string,
  payload: UpdateFoundItemInput,
  userId: string,
) => {
  const item = await getOwnedFoundItem(id, userId);
...
export const deleteFoundItem = async (id: string, userId: string) => {
  const item = await getOwnedFoundItem(id, userId);
  await FoundItem.deleteOne({ _id: item._id });
};

export const resolveFoundItem = async (id: string, userId: string) => {
  const item = await getOwnedFoundItem(id, userId);
...
export const getFoundItemsStatistics = async (userId?: string) => {
  const filter: Record<string, unknown> = {};
  ...
  const [totalItems, activeItems, resolvedItems, categoryStats, recentItems] =
    await Promise.all([
      FoundItem.countDocuments(filter),
      FoundItem.countDocuments({ ...filter, status: "active" }),
      FoundItem.countDocuments({ ...filter, status: "resolved" }),
      FoundItem.aggregate([
        { $match: filter },
